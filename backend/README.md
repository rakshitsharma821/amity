...
The Railway launcher defaults `PUBLIC_DEMO_MODE` to `true` for the public hackathon demo, so judges can use the scanner proxy without signing in. In this mode, the backend refuses every target except the bundled API at `http://127.0.0.1:4000`, even if an allowlist is configured. Leave `AUTHORIZED_TARGET_HOSTS` empty. Do not use public demo mode for customer data or general production scanning; set `PUBLIC_DEMO_MODE=false` and configure Supabase operator authentication for that use. The Railway launcher pins `SCANNER_API_URL` to the co-hosted backend at `http://127.0.0.1:5000`; local development URLs must not be used for this single-service deployment.
