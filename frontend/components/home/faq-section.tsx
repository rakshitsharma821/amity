  ...
  const faqs = [
    {
      ...
      a: 'Zero-trust security requires accountability. To prevent VANGAURD-API from ever being turned into an attack tool against third-party systems, users must prove domain control via either a DNS TXT record or a /.well-known verification file. Only confirmed targets can be audited.',
    },
    {
      q: 'Why does VANGAURD-API require credentials for two test accounts?',
    ...
    {
      q: 'Can VANGAURD-API run inside CI/CD pipelines to block vulnerable PRs?',
      a: 'Yes. The VANGAURD-API CLI engine includes a --fail-on flag (e.g. --fail-on high), which exits with a non-zero exit code if high-severity authorization flaws or unmasked card exposures are detected, cleanly failing the build in GitHub Actions, GitLab CI, or CircleCI.',
    },
    {
      q: 'Will the scanner overwhelm or degrade my API server?',
      a: 'No. VANGAURD-API enforces bounded request bursts (maximum 30-50 requests per endpoint during rate-limit evaluations) and low concurrency to ensure your sandbox or staging environments remain fully operational throughout the audit.',
    },
    {
