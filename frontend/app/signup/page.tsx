...
import { Shield, Mail, Lock, Eye, EyeOff, AlertCircle, CheckCircle2, Zap } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleOAuth = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      if (error) {
        setError(`OAuth Notice: ${error.message}. If provider is not enabled in Supabase, use 1-Click Instant Demo Login.`);
      }
    } catch {
      setError('OAuth provider is not enabled in Supabase Console. Please use 1-Click Instant Demo Login.');
    } finally {
      setLoading(false);
    }
  };
