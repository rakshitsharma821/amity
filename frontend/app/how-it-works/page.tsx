...
[AUTH] Detected SecurityScheme: HTTP Bearer (JWT)`,
    ...
    },
    {
      ...
      icon: Users,
      summary:
        'To accurately test authorization, SentinelAPI provisions sessions for two separate test entities: User A (the requester/attacker context) and User B (the victim/resource owner context).',
...
[SARIF] Export completed: reports/sentinel-audit.json`,
      details: [
    ...
    <div className="relative pt-32 pb-24 max-w-5xl mx-auto px-6">
      ...
      >
        ...
        <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-muted-heading mb-4">
          How SentinelAPI verifies authorization boundaries.
        </h1>
      ...
      >
        ...
        <div>
          <strong className="text-white">Mandatory Pre-Flight Guardrail:</strong> Before any scan is dispatched, our platform requires domain ownership verification via DNS TXT or <code className="bg-black/40 px-1 py-0.5 rounded text-white font-mono">/.well-known/sentinelapi-verify.txt</code>. SentinelAPI cannot be pointed at arbitrary third-party targets.
        </div>
      </motion.div>
